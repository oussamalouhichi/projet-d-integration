...
def dashboard(request):
    ...
    if address_query:
        all = all.filter(immeuble__address__icontains=address_query)
    # If no offices are available, create a sample one for display
    if not all.exists():
        # Check if we have any buildings first
        buildings = Immeuble.objects.all()
        if not buildings.exists():
            # Create a sample building
            sample_building = Immeuble(
                address="123 Sample Street",
                nbre_bureau=5
                # image field is intentionally left empty
            )
            sample_building.save()
        else:
            sample_building = buildings.first()
        # Only create a sample office if there is no office with num=101
        from django.core.files.base import ContentFile
        from django.core.files.storage import default_storage
        if not Office.objects.filter(num=101).exists():
            sample_office = Office(
                immeuble=sample_building,
                etage="1ere niveau",
                num=101,
                rent_amount=1500,
                is_available=True
                # picture fields are intentionally left empty
            )
            # Save without validation to avoid image field requirements
            sample_office.save(force_insert=True)
    ...
    return render(request, 'loyer/index.html', {'listing': listing, 'address_query': address_query})
...
@login_required 
def immeuble_detail(request, immeuble_id):
    immeuble = get_object_or_404(Immeuble, id=immeuble_id)
    return render(request, 'loyer/immeuble_detail.html', {'immeuble': immeuble})
